class Solution {
   ...
   private:
    void recLoop(vector<int>& nums, int target, int idx, vector<int>& curr,
                 vector<vector<int>>& result) {
        if (target == 0) {
        ...
        for (int i = idx; i < nums.size(); i++) {
            ...
            recLoop(nums, target - nums[i], i, curr, result);  // same i → reuse allowed // explore
            curr.pop_back(); // undo
        }
    }

    void recTakeNotTake(vector<int>& nums, int target, int idx, vector<int>& curr,
                        vector<vector<int>>& result) {
        if (target == 0) {
            result.push_back(curr);
            return;
