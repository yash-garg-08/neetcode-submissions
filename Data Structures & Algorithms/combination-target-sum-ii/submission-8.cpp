...
private:
    void recLoop(vector<int>& candidates, int target, int idx,
                 vector<int>& curr, vector<vector<int>>& result)
    {
        ...
        for (int i = idx; i < candidates.size(); i++) {
            ...
            recLoop(candidates, target - candidates[i], i + 1, curr, result); // i+1 → no reuse
            curr.pop_back();
        }
    }

    void recTakeNotTake(vector<int>& candidates, int target, int idx,
                         vector<int>& curr, vector<vector<int>>& result)
    {
        if (target == 0) {
            result.push_back(curr);
